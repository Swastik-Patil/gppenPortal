...
function MyLcApplications() {
  ...
  function readUserPastData() {
    const db = dbref(database);
    const IDRef = currentUser.uid;
    get(child(db, `/lchistory/rejected/${IDRef}`))
      .then((snapshot) => {
        if (snapshot.exists()) {
                    ...
                    <Tr key={keyIndex}>
                      ...
                      <Td>
                        {new Date(pendingData.appliedDate).getDate() +
                          "/" +
                          new Date(pendingData.appliedDate).getMonth() +
                          1 +
                          "/" +
...
export default MyLcApplications;
